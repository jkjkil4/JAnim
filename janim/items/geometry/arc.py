...
from janim.utils.space_ops import angle_between_vectors, angle_of_vector, get_norm
...
class Arc(GeometryShape, ArcCenter):
    ...
    def _reshape(
        ...
        start_angle, angle, radius, n_components = self._reshape_memorize(
            start_angle=start_angle, angle=angle, radius=radius, n_components=n_components
        )
...
class Cmpt_VPoints_CircleImpl[ItemT](Cmpt_VPoints[ItemT], impl=True):
    ...
    def surround(
        ...
        *,
        ...
        self.replace(
            item, dim_to_match, stretch=stretch, root_only=root_only, item_root_only=item_root_only
        )
    ...
    def at_angle(self, angle: float) -> np.ndarray:
        ...
        """
        return self.pfp((angle - self.start_angle) / TAU % 1)
...
class SmallDot(Dot):
    ...
    def __init__(
        self, point: np.ndarray = ORIGIN, radius: float = DEFAULT_SMALL_DOT_RADIUS, **kwargs
    ) -> None:
...
class AnnularSector(GeometryShape, ArcCenter):
    ...
    def _reshape(
        ...
        inner_arc, outer_arc = (unit * radius for radius in (inner_radius, outer_radius))
...
class Sector(Arc):
    ...
    def reshape(
        ...
        *,
        ...
        super().reshape(
            start_angle, angle, radius, n_components=n_components, arc_center=arc_center
        )
        self._reshape_additional_corners()
        return self
...
class Annulus(GeometryShape):
    ...
    def _reshape(
        ...
        inner, outer = (unit * radius + arc_center for radius in (inner_radius, outer_radius))
        outer = outer[::-1]
