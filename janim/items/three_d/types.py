...
type Resolution = int | Sequence[int]
...
class SurfaceFace(Polygon):
    def __init__(self, u1: float, u2: float, v1: float, v2: float, u_index: int, v_index: int, **kwargs):
        super().__init__(
            [u1, v1, 0],
            [u2, v1, 0],
            [u2, v2, 0],
...
class CheckerboardSurface[T: SurfaceGeometry](Group[SurfaceFace], VItem):
    ...
    def _get_uv_faces(self) -> list[SurfaceFace]:
        u_values, v_values = _get_u_values_and_v_values(self.geometry.u_range, self.geometry.v_range, self.resolution)
        return [
