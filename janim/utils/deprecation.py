...
_DEPRECATED_MSG_I = _(
    '{name!r} is deprecated and will be removed in JAnim {remove}, use {instead!r} instead'
)
...
def is_removed(remove: tuple[int, int]):
    return (_version_tuple[0] > remove[0]) or (
        _version_tuple[0] == remove[0] and _version_tuple[1] >= remove[1]
    )
