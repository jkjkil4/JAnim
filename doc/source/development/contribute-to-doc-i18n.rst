...
..    注意不能直接将以下命令的 `python` 换为虚拟环境中的 python 路径，需要真正激活虚拟环境后才能完美运行命令
...
.. 4. 使用 `build-docs` 命令在本地构建文档预览翻译效果
...
.. note::
    ...
    .. code-block:: text
        ...
        请完成 `doc/source/locales/<language>/LC_MESSAGES/` 目录下的 `.po` 文件中的 `msgstr` 翻译工作
